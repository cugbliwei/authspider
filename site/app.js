...
function getData(param) {
    $.ajax({
        ...
        success: function(data){
            id = data.id;
            printTime();
            if (data.status == "output_verifycode" || data.status == "wrong_verifycode" || data.status=='need_scan_qr'){
                $('#result').empty();
                $('#result').html("");
            ...
            else if(data.status == "need_param" || data.status == "wrong_second_password") {
                ...
                getData({id: data.id, username: username, password: encrypted, name: name, idNumber: idNumber})
